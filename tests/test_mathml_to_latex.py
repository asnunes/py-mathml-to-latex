...
class TestMathMLToLaTeX(unittest.TestCase):
    ...
    def test_convert_mfenced_as_partial_function(self):
        expected_latex = r"f \left(x\right) = \left{\begin{matrix} x^{2} , x < 0 \\ e^{x} , x \geq 0 \end{matrix}\right"
        result = self.converter.convert(mfenced_as_partial_function)
        self.assertEqual(result, expected_latex)

    def test_convert_mfenced_with_nested_mtables(self):
        expected_latex = (
            r"\begin{bmatrix} \begin{matrix}a_{11} & a_{12}\end{matrix} & "
            r"\begin{matrix}\ldots & \ldots\end{matrix} & a_{1 n} \\ "
            r"\begin{matrix}a_{21} & a_{22}\end{matrix} & "
            r"\begin{matrix}\ddots & \end{matrix} & a_{2 n} \\ "
            r"\begin{matrix}\begin{matrix}\vdots & \vdots\end{matrix} \\ "
            r"\begin{matrix}a_{m 1} & a_{m 2}\end{matrix}\end{matrix} & "
            r"\begin{matrix}\begin{matrix} & \ddots\end{matrix} \\ "
            r"\begin{matrix}\ldots & \ldots\end{matrix}\end{matrix} & "
            r"\begin{matrix}\vdots \\ a_{m n}\end{matrix} \end{bmatrix}"
        )
        result = self.converter.convert(mfenced_with_nested_mtables)
        self.assertEqual(result, expected_latex)

    def test_convert_mfrac_simple(self):
        expected_latex = r"\frac{x}{3}"
        result = self.converter.convert(mfrac)
        self.assertEqual(result, expected_latex)

    def test_convert_mfrac_with_mrow(self):
        expected_latex = r"\frac{a + 2}{b - 3}"
        result = self.converter.convert(mfrac_with_mrow)
        self.assertEqual(result, expected_latex)

    def test_convert_short_m_frac(self):
        expected_latex = r"1/\left(x^{3} + 3\right)"
    ...
    def test_convert_maction_toggle(self):
        expected_latex = r"a + 2 \Longrightarrow b - 3"
        result = self.converter.convert(maction)
        self.assertEqual(result, expected_latex)

    def test_convert_maction_with_mrow(self):
        expected_latex = r"a + 2 \Longrightarrow b - 3 \Longrightarrow a + b"
    ...
    def test_convert_menclose_default(self):
        expected_latex = r"\overline{\left.\right)a + 2}"
    ...
    def test_convert_mover_double_mrow(self):
        expected_latex = r"\overset{a + b}{x + y + z}"
    ...
    def test_convert_mmultiscript(self):
        expected_latex = r"\left(N a\right)_{11}^{+}"
    ...
    def test_convert_ms_word_input(self):
        expected_latex = (
            r"V_{i} \frac{\Delta C_{A , i}^{t}}{\Delta t} = "
            r"\sum_{j = k}^{N} G_{i , j}^{D} \left(C_{A , j} - C_{A , i}\right)"
        )
        result = self.converter.convert(ms_word_input)
        self.assertEqual(result, expected_latex)

    def test_convert_not_add_unnecessary_delimiters(self):
        expected_latex = r"\text{Required Value}_{\text{other}} \geq \frac{21 f t^{3}}{A C H} \cdot \left(\right. \frac{I_{o}}{1000 B_{\text{Btu} / \text{h}}} \left.\right)"
