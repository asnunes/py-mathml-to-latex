...
from src.mathml_to_tex.protocols import MathMLElement, GenericMathMLElement
...
class ElementsToMathMLAdapter:
    def convert(self, els: List[DOMElement]) -> List[MathMLElement]:
        ...
        """
        return [
            self._convert_element(el)
            for el in els
            if el.tagName is not None
        ]
    ...
    def _convert_element(self, el: DOMElement) -> MathMLElement:
        ...
        value = '' if has_children else (el.firstChild.nodeValue if el.firstChild else '')

        children = (
            self.convert([child for child in el.childNodes if child.nodeType == Node.ELEMENT_NODE])
